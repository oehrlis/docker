...
EXPECTED_HOST="cdbencs1"
WAIT_FOR_HOST="cdbencs2"
...
while true; do
    if [[ -f "$STATUS_FILE" ]]; then
        ...
        if [[ "$STATUS" == "done" ]]; then
            echo "${WAIT_FOR_HOST}: Configuration is complete. Proceeding..."
            break
        elif [[ "$STATUS" == "running" ]]; then
            echo "${WAIT_FOR_HOST}: Still configuring. Waiting ${WAIT_TIME}s ..."
        else
            echo "${WAIT_FOR_HOST}: Unknown status in lock file. Exiting."
            exit 1
        fi
    else
        echo "${WAIT_FOR_HOST}: Lock file not found. Waiting ${WAIT_TIME}s ..."
    fi
    sleep $WAIT_TIME
done
