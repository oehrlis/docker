...
----------------------------------------------------------------------------
--  Name......: password_hash_10g.sql
--  Author....: Stefan Oehrli (oes) stefan.oehrli@trivadis.com
--  Editor....: Stefan Oehrli
--  Date......: 2019.11.29
--  Usage.....: @password_hash_10g
...
DECLARE
    ...
    SELECT '&pwd' INTO vPass FROM DUAL;
    -- Build the new userpwd String as multibyte with the high byte set to 0 and convert the string into raw
    FOR i IN 1..length(upper(vUser||vPass)) LOOP vUniStr := vUniStr||CHR(0)||SUBSTR(UPPER(vUser||vPass),i,1); END LOOP;
    ...
    dbms_output.put_line('User     : '||vUser||' ('||vPass||')');
    dbms_output.put_line('Hash 10g : '||hextoraw(substr(vEncRAW,(length(vEncRAW)-16+1),16)));
    dbms_output.put_line('----------------------------------------------------------');
END;
/
-- EOF ---------------------------------------------------------------------
