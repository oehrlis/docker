# Oracle 19.1.0.0 Database

A sample docker compose file to create an Oracle 19.1.0.0 database. The persistent data will be stored on an external volume. By default the volume will be created in the directory specified by the environment variable *DOCKER_VOLUME_BASE*. If the environment variable is not specified, it will use the default value from ``*.env`` which is the current path. Beside the usual changes e.g. container name, hostname, ports etc. you can configure how the DB itself will be created by specify several configuration parameter.

| Environment variable   | Value / Directories                            | Comment                                                                                                                                                                                                                                                                                                                                                       |
|------------------------|------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ORACLE_SID             | ``TDB191S``                                    | Default Oracle SID. Usually it will default to the variable which has been specified during build. A custom SID can / should be specified.                                                                                                                                                                                                                    |
...
Create the container and start the database. The startup script will verify if there has been an Oracle database created in the volume. If there is no database a new database will be created based on the dbca templates. All scripts (SQL or Bash) located in the foler ``config/setup`` will be executed after the database is created during initial startup. Afterwards you still can access them via ``/u01/config``.

To just create one container with a single tenant DB use docker-compose and specify the service.

```bash
docker-compose up -d tdb191s
```

Alternatively specify the service to create a container with a simple CDB.

```bash
docker-compose up -d tdb191c
```

Or create / start both containers
...
```bash
---------------------------------------------------------------
 - DATABASE TDB191S IS READY TO USE!
---------------------------------------------------------------
```
