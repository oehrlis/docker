...
# Date.......: 2018.03.13
# Revision...:  
# Purpose....: Build script to build Oracle Java 1.7
...
DOCKER_MAJOR_IMAGE_NAME="serverjre:7"
...
SERVER_JRE_PACKAGE="server-jre-7u${JAVA_UPDATE}-linux-x64.tar.gz"
...
if [ -f "${DOCKER_BUILD_DIR}/${SERVER_JRE_PACKAGE}" ]; then
    echo "Did found a server JRE 1.7 package ${SERVER_JRE_PACKAGE}"
elif [ -f "${DOCKER_BUILD_DIR}/${JAVA_PKG}" ]; then
    echo "Did found server JRE 1.7 patch package ${JAVA_PKG}"
    echo "Unzip ${JAVA_PKG}"
